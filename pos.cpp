...
void tambahProduk(const std::string& nama, double harga, int stok) {
    std::string sql = "INSERT INTO produk (nama, harga, stok) VALUES ('" + nama + "', " +
...
void tampilkanProduk() {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        std::cout << "ID\tNama\t\tHarga (Rp)\tStok\n";
        std::cout << "----------------------------------------\n";
        ...
        std::cerr << "Gagal menampilkan produk: " << sqlite3_errmsg(db) << "\n";
    }
}

void perbaruiProduk(int id, double harga, int stok) {
...
void hapusProduk(int id) {
    std::string sql = "DELETE FROM produk WHERE id = " + std::to_string(id) + ";";
    if (sqlite3_exec(db, sql.c_str(), nullptr, nullptr, &pesanError) != SQLITE_OK) {
...
void tambahKeKeranjang(int idProduk, int jumlah) {
    std::string sql = "SELECT nama, harga, stok FROM produk WHERE id = " + std::to_string(idProduk) + ";";
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
    ...
    if (pembayaran >= total) {
        ...
        for (const auto& item : keranjang) {
            ...
            std::string updateStok = "UPDATE produk SET stok = stok - " + std::to_string(item.jumlah) +
                                     " WHERE id = " + std::to_string(item.idProduk) + ";";
